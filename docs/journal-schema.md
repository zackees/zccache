...
| `tree_peak_rss_bytes` | integer | no            | Largest sampled resident total of a compiler/tool child **plus its live descendants**, in bytes (zackees/zccache#1588). Unlike `child_peak_rss_bytes` it includes a linker grandchild (`rustc` -> `cc` -> `ld`). Each 250 ms sample sums current RSS (Linux `VmRSS` over `/proc/<pid>/task/*/children`, macOS `ri_resident_size` over `proc_listchildpids`, Windows `WorkingSetSize` over a Toolhelp32 snapshot) across at most 4096 processes; the field is the maximum sample, so a spike between samples can be missed. Omitted for hits and when unmeasurable. |
| `tree_peak_rss_source` | string | no            | How `tree_peak_rss_bytes` was obtained. Currently always `sampled`; present only with `tree_peak_rss_bytes`. |
