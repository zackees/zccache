...
//! |---------------------------------------------------|----------------|-----------|
//! | `kernal_api::broker::protocol::ENVELOPE_VERSION` | **keep on v1** | The probe path itself is a v1 protocol artifact (`BackendHandle` lives on the v1 broker namespace); the v2 broker uses Frame streaming (`OPEN/DATA/CLOSE/…`) instead of the v1 envelope shape. There is no v2 envelope to migrate this to. |
//! | `kernal_api::broker::protocol::MAX_FRAME_BYTES`  | **keep on v1** | Same reasoning — this is the cap on the v1 envelope body length; the v2 streaming layer has its own per-stream windowing primitives. |
//! | `kernal_api::broker::protocol::Frame`           | **keep on v1** | The probe message is itself a v1 `Frame` payload (kind `FRAME_KIND_REQUEST`, payload protocol `BACKEND_HANDLE_PROBE_PAYLOAD_PROTOCOL`). Migrating the *probe* to v2 is a separate, larger migration that replaces the whole probe contract with a v2-streaming equivalent (tracked in zccache#782 Phase B). |
//!
    ...
    W: AsyncWrite + Unpin,
{
    ...
    let running_process_version = kernal_api::broker::protocol::ENVELOPE_VERSION;
    ...
    if body_len > kernal_api::broker::protocol::MAX_FRAME_BYTES {
    ...
    let frame = kernal_api::broker::protocol::Frame::decode(&read_buf[5..5 + body_len])
...
fn is_backend_handle_probe_request(frame: &kernal_api::broker::protocol::Frame) -> bool {
    use kernal_api::broker::protocol::{FrameKind, PayloadEncoding};
    use kernal_api::broker::protocol_v2::backend_handle::BACKEND_HANDLE_PROBE_PAYLOAD_PROTOCOL;
...
fn backend_handle_probe_response(
    request: &kernal_api::broker::protocol::Frame,
    daemon: &kernal_api::broker::protocol_v2::backend_handle::DaemonProcess,
) -> Result<kernal_api::broker::protocol::Frame, IpcError> {
    use kernal_api::broker::protocol::{Frame, FrameKind, PayloadEncoding};
    use kernal_api::broker::protocol_v2::backend_handle::BACKEND_HANDLE_PROBE_PAYLOAD_PROTOCOL;
...
async fn write_running_process_frame<W>(
    writer: &mut W,
    frame: &kernal_api::broker::protocol::Frame,
) -> Result<(), IpcError>
where
    W: AsyncWrite + Unpin,
{
    ...
    if body.len() > kernal_api::broker::protocol::MAX_FRAME_BYTES {
        return Err(IpcError::Endpoint(format!(
    ...
    writer
        .write_all(&[kernal_api::broker::protocol::ENVELOPE_VERSION])
        .await?;
...
#[cfg(test)]
mod tests {
    use kernal_api::broker::protocol::Endpoint;
    use prost::Message;
    ...
    fn probe_reply_leaves_the_legacy_sha256_identity_empty() {
        ...
        let request = kernal_api::broker::protocol::Frame {
            payload: nonce.clone(),
            ..Default::default()
        };
