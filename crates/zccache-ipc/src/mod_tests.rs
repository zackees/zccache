...
impl EnvGuard {
    fn set_cache_dir(value: &std::path::Path) -> Self {
        ...
        let previous_cache_dir = std::env::var_os(zccache_core::config::CACHE_DIR_ENV);
        let previous_namespace = std::env::var_os(zccache_core::config::DAEMON_NAMESPACE_ENV);
        ...
        std::env::remove_var(zccache_core::config::DAEMON_NAMESPACE_ENV);
        Self {
            _lock: lock,
            previous_cache_dir,
            previous_namespace,
    ...
    fn set_cache_dir_and_namespace(value: &std::path::Path, namespace: &str) -> Self {
        ...
        let previous_running_process_disable = std::env::var_os(RUNNING_PROCESS_DISABLE_ENV);
        std::env::set_var(zccache_core::config::CACHE_DIR_ENV, value);
        std::env::set_var(zccache_core::config::DAEMON_NAMESPACE_ENV, namespace);
        Self {
            _lock: lock,
            previous_cache_dir,
            previous_namespace,
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        ...
        match &self.previous_namespace {
            Some(value) => std::env::set_var(zccache_core::config::DAEMON_NAMESPACE_ENV, value),
            None => std::env::remove_var(zccache_core::config::DAEMON_NAMESPACE_ENV),
        }
...
fn test_daemon_status(endpoint: &str) -> zccache_protocol::DaemonStatus {
    zccache_protocol::DaemonStatus {
        version: zccache_core::VERSION.to_string(),
        daemon_namespace: "test".to_string(),
        endpoint: endpoint.to_string(),
        private_daemon: zccache_protocol::PrivateDaemonStatus::shared(),
        artifact_count: 0,
        cache_size_bytes: 0,
        metadata_entries: 0,
        uptime_secs: 1,
    ...
    let server = tokio::spawn(async move {
        ...
        match msg {
            Some(zccache_protocol::DecodedWireMessage::ProstV16(request)) => {
                ...
                assert!(matches!(
                    request.body,
                    Some(zccache_protocol::wire_prost::zccache_v1::request::Body::Status(_))
                ));
    ...
    let server = tokio::spawn(async move {
        ...
        match msg {
            Some(zccache_protocol::DecodedWireMessage::ProstV16(request)) => {
                ...
                assert!(matches!(
                    request.body,
                    Some(zccache_protocol::wire_prost::zccache_v1::request::Body::Clear(_))
                ));
