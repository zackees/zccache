...
#[must_use]
pub fn response_to_prost(response: &crate::Response, request_id: &str) -> zccache_v1::Response {
    use zccache_v1::response::Body;

    let body = match response {
        crate::Response::Pong => Body::Pong(zccache_v1::Empty {}),
        crate::Response::ShuttingDown => Body::ShuttingDown(zccache_v1::Empty {}),
        crate::Response::Status(status) => Body::Status(daemon_status_to_prost(status)),
        crate::Response::LookupResult(result) => Body::LookupResult(lookup_result_to_prost(result)),
        crate::Response::StoreResult(result) => Body::StoreResult(zccache_v1::StoreResult {
            kind: store_result_kind_to_prost(result).into(),
        }),
        crate::Response::SessionStarted {
            session_id,
            journal_path,
        ...
        crate::Response::SessionEnded { stats } => Body::SessionEnded(zccache_v1::SessionEnded {
            stats: stats.as_ref().map(session_stats_to_prost),
        }),
        crate::Response::LinkResult {
            exit_code,
            stdout,
            stderr,
            cached,
        ...
        crate::Response::Error { message } => Body::Error(zccache_v1::Error {
        ...
        crate::Response::SessionStatsResult { stats } => {
            Body::SessionStatsResult(zccache_v1::SessionStatsResult {
                stats: stats.as_ref().map(session_stats_to_prost),
            })
        }
        crate::Response::FingerprintCheckResult {
            decision,
            reason,
            changed_files,
        ...
        crate::Response::FingerprintAck => Body::FingerprintAck(zccache_v1::Empty {}),
        crate::Response::RustArtifactList { artifacts } => {
            Body::RustArtifactList(zccache_v1::RustArtifactList {
                artifacts: artifacts.iter().map(rust_artifact_info_to_prost).collect(),
            })
        }
        crate::Response::GenericToolExecResult {
            exit_code,
            stdout,
            stderr,
            output_files,
...
pub fn response_from_prost(response: zccache_v1::Response) -> Result<crate::Response, String> {
    use zccache_v1::response::Body;

    match response.body {
        Some(Body::Pong(_)) => Ok(crate::Response::Pong),
        Some(Body::ShuttingDown(_)) => Ok(crate::Response::ShuttingDown),
        Some(Body::Status(status)) => daemon_status_from_prost(status).map(crate::Response::Status),
        Some(Body::LookupResult(result)) => {
            lookup_result_from_prost(result).map(crate::Response::LookupResult)
        }
        Some(Body::StoreResult(result)) => {
            store_result_kind_from_prost(result.kind).map(crate::Response::StoreResult)
        }
        Some(Body::SessionStarted(started)) => Ok(crate::Response::SessionStarted {
            session_id: started.session_id,
            journal_path: started.journal_path.map(path_from_prost),
        }),
        Some(Body::CompileResult(result)) => Ok(crate::Response::CompileResult {
            exit_code: result.exit_code,
            stdout: std::sync::Arc::new(result.stdout),
            stderr: std::sync::Arc::new(result.stderr),
            cached: result.cached,
        }),
        Some(Body::SessionEnded(ended)) => Ok(crate::Response::SessionEnded {
            stats: ended.stats.map(session_stats_from_prost),
        }),
        Some(Body::LinkResult(result)) => Ok(crate::Response::LinkResult {
            exit_code: result.exit_code,
            stdout: std::sync::Arc::new(result.stdout),
            stderr: std::sync::Arc::new(result.stderr),
            cached: result.cached,
        ...
        Some(Body::Error(error)) => Ok(crate::Response::Error {
            message: error.message,
        }),
        Some(Body::Cleared(cleared)) => Ok(crate::Response::Cleared {
        ...
        Some(Body::SessionStatsResult(result)) => Ok(crate::Response::SessionStatsResult {
            stats: result.stats.map(session_stats_from_prost),
        }),
        Some(Body::FingerprintCheckResult(result)) => Ok(crate::Response::FingerprintCheckResult {
            decision: result.decision,
            reason: result.reason,
            changed_files: result.changed_files,
        }),
        Some(Body::FingerprintAck(_)) => Ok(crate::Response::FingerprintAck),
        Some(Body::RustArtifactList(list)) => Ok(crate::Response::RustArtifactList {
            artifacts: list
                .artifacts
                .into_iter()
        ...
        Some(Body::GenericToolExecResult(result)) => Ok(crate::Response::GenericToolExecResult {
            exit_code: result.exit_code,
            stdout: std::sync::Arc::new(result.stdout),
            stderr: std::sync::Arc::new(result.stderr),
            output_files: result
        ...
        Some(Body::Backpressure(backpressure)) => Ok(crate::Response::Backpressure {
        ...
        Some(Body::ReleaseWorktreeHandlesResult(result)) => {
            Ok(crate::Response::ReleaseWorktreeHandlesResult {
