...
warm, rust-plan, fp, wrap, mtime (snapshot/replay), etc.) so no single file
is too large to navigate. `warm_delivery.rs` delivers each `zccache warm`
payload per `ZCCACHE_MODE` (#1683).
