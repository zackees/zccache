...
pub(crate) fn cmd_configure(
    ...
    let mut command = std::process::Command::new(&meson_bin);
    command.arg("setup").arg(&build_abs).arg(&source_abs);
...
fn capture_meson_version(meson_bin: &Path) -> std::io::Result<String> {
    let out = kernal_api::run_bounded_command(
        kernal_api::SpawnSpec::new(meson_bin).arg("--version"),
        std::time::Duration::from_secs(30),
        1024 * 1024,
    )
    .map_err(|error| match error {
        kernal_api::BoundedProcessError::Spawn(error)
        | kernal_api::BoundedProcessError::Io(error) => error,
        kernal_api::BoundedProcessError::TimedOut => std::io::Error::new(
            std::io::ErrorKind::TimedOut,
            "meson version probe timed out",
        ),
        kernal_api::BoundedProcessError::OutputLimitExceeded { .. } => std::io::Error::new(
            std::io::ErrorKind::FileTooLarge,
            "meson version probe exceeded output limit",
        ),
    })?;
    if out.exit.raw_code() != 0 {
