...
#[test]
fn tree_peak_rss_serializes_with_its_sampled_source() {
    let measured = JournalEntry::new(
        super::make_ctx(vec!["--crate-name", "a"]),
        "miss",
        0,
    ...
    .with_child_memory(super::super::ChildMemory {
        peak_rss_bytes: Some(100),
        tree_peak_rss_bytes: Some(5_000),
    });
    let v: serde_json::Value = serde_json::to_value(&measured).unwrap();
    assert_eq!(v["child_peak_rss_bytes"], 100);
    assert_eq!(v["tree_peak_rss_bytes"], 5_000);
    ...
    .with_child_memory(super::super::ChildMemory::default());
    let json = serde_json::to_string(&hit).unwrap();
    assert!(!json.contains("tree_peak_rss"), "json: {json}");
}
