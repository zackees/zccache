...
/// soldr#3152 / zccache#1586: the compiler session samples the child's own
/// memory high-water mark and hands it to the enclosing compile scope, which
/// is what puts `child_peak_rss_bytes` on the journal row.
#[cfg(unix)]
#[tokio::test]
async fn compiler_session_records_child_peak_rss_into_the_compile_scope() {
    let builder = kernal_api::SpawnSpec::new("/bin/sh").args(["-c", "sleep 1"]);
    let (output, _, _, memory) =
        crate::daemon::compile_journal::capture_miss_reason(Box::pin(async move {
            async_builder_output_with_priority_decision(
                builder,
                CompilePriority::Normal,
                "peak-rss fixture".to_string(),
            )
            .await
            .0
        }))
        .await;
    assert!(output.expect("fixture runs").status.success());
    let peak = memory
        .peak_rss_bytes
...
#[cfg(unix)]
#[tokio::test]
async fn compiler_session_records_a_tree_peak_that_covers_a_heavy_descendant() {
    let builder = kernal_api::SpawnSpec::new("/bin/sh").args([
        "-c",
