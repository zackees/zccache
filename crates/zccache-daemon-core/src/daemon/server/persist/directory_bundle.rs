...
fn set_timestamp(path: &Path, seconds: u64, nanos: u32) -> std::io::Result<()> {
    ...
    kernal_api::platform::fs::set_file_mtime(path, kernal_api::platform::fs::FileTime::from_system_time(modified))
}
...
mod tests {
    ...
    fn directory_bundle_round_trip_preserves_tree_and_mtime() {
        ...
        let root_mtime = kernal_api::platform::fs::FileTime::from_unix_time(1_600_000_000, 765_432_100);
        ...
        assert_eq!(
            kernal_api::platform::fs::FileTime::from_last_modification_time(&std::fs::metadata(restored).unwrap()),
            mtime
        );
        assert_eq!(
            kernal_api::platform::fs::FileTime::from_last_modification_time(&std::fs::metadata(target).unwrap()),
            root_mtime
        );
    }
