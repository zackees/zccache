...
/// `<crate>@<toolchain>` sidecar — `lib`-prefixed on unix, bare on Windows —
/// in [`dylint_library_sidecar_output_path`]).
///
...
/// evaluated on either side of the IPC boundary (that crate parses raw argv
/// during admission, before a request even reaches the daemon; this one
/// re-derives it from the daemon's own `RustcParsedArgs`), so widening one
/// without the other makes the compile cacheable but its output set
/// incomplete. Keep both gates in lockstep: soldr#2349 removed the
/// non-Windows-only restriction from both sides together.
...
/// inline.
///
/// Mirrors `RustcHost` in `zccache-compiler/src/parse_rustc.rs`, which
/// threads the same fact through the wrapper-side naming helpers for the
/// same reason (see the mirroring note on `is_dylint_cdylib_args` above):
/// `is_windows()`/`is_macos()` are compile-time constants, so a function
/// reading them directly can only be exercised from the one host actually
/// running the test. Threading the value in keeps the sidecar-name
/// derivation testable for all three families from a single CI runner.
...
/// The `<crate>@<toolchain>` copy `dylint-link` writes beside the lint
/// cdylib — `lib`-prefixed on unix, bare on Windows — the second half of
/// the Dylint output set, without which a replayed hit would leave dylint
/// unable to load the library it just "built".
///
