//! Process entry point shared by the legacy download-daemon binary and the
//! `zccache` multicall binary.

use clap::Parser;

use crate::download_protocol::daemon_mgmt;
...
/// Run the download daemon command-line surface in the current process.
pub fn run() {
    let args = Args::parse();
    if args.foreground {
...
fn print_status(args: &Args) {
    ...
    match query_daemon_status(&endpoint) {
        ...
        Err(_) => println!("  status:     not running"),
    }
}
